...
extension CustomUIView {
    ...
    private func setupCloseSFSymbol() {
        closeSFSymbol.tintColor = Styleguide.getPrimaryColor()
        addSubview(closeSFSymbol)
        closeSFSymbol.centerXToSuperview()
        closeSFSymbol.topToBottom(of: label, offset: kPadding)
        closeSFSymbol.width(kButtonDimension)
        closeSFSymbol.height(kButtonDimension)
    }
}
